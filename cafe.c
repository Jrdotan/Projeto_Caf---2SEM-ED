...
// inicializando frente e trás das filas
    ...
    while (curr != NULL) {
        if (curr->id == id) {
           ...
           while(mes < 0 || mes > 12){
            ...
            this->next = NULL; // Gambiarra feita pra verificar se a próxima estrutura está vazia
...
void ler_participantes() {
    ...
    while (fscanf(handler, "%d,%[^,],%[^,],%d\n", &id, nome, curso, &ano) == 4) // Gambiarra número 2 que precisei fazer para verificar se só os 4 inputs pedidos estão sendo passados por participantes
  {
...
void ler_contribuintes() {
    ...
    while (fscanf(handler, "%d,%f,%d,%d\n", &id, &valor, &mes, &ano) == 4) // Gambiarra número 2.5 que precisei fazer para verificar se só os 4 inputs pedidos estão sendo passados por participantes
  {
...
void carregar_participantes() {
    ...
    while (fscanf(handler, "%d,%[^,],%[^,],%d\n", &id, nome, curso, &ano) == 4) {
        ...
        if (novo_participante == NULL) {
            printf("Erro ao alocar mem�ria para participante.\n");
            fclose(handler);
            liberar_memoria();
            exit(1);
        }
